...
describe("debug-levels", function()
  ...
  describe("Const arg builders", function()
    ...
    it("get_record_get_args", function()
      ...
      eq("User", args[5])
      eq("-w", args[6])
      eq("Username='test@example.com'", args[7])
      eq("-o", args[8])
      eq("test@example.com", args[9])
      eq("--json", args[10])
      eq(10, #args)
    end)
    ...
    it("get_query_args includes -t (tooling API)", function()
      ...
      eq("query", args[2])
      eq("-q", args[3])
      eq("SELECT Id FROM DebugLevel", args[4])
      eq("-o", args[5])
      eq("test@example.com", args[6])
      eq("-t", args[7])
      eq("--json", args[8])
      eq(8, #args)
    end)
    ...
    it("get_record_create_args builds DebugLevel record create", function()
      ...
      eq("record", args[3])
      eq("-o", args[4])
      eq("test@example.com", args[5])
      eq("-s", args[6])
      eq("DebugLevel", args[7])
      eq("-t", args[8])
      eq("-v", args[9])
      eq("ApexCode=Fine DeveloperName=Test", args[10])
      eq("--api-version", args[11])
      eq("65.0", args[12])
      eq("--json", args[13])
      eq(13, #args)
    end)
    ...
    it("get_record_update_args includes record id", function()
      ...
      eq("-t", args[8])
      eq("-v", args[9])
      eq("ApexCode=Fine", args[10])
      eq("-i", args[11])
      eq("7dl00000001", args[12])
      eq("--api-version", args[13])
      eq("65.0", args[14])
      eq("--json", args[15])
      eq(15, #args)
    end)
  ...
  describe("DebugUtils.parse_user_data", function()
    ...
    it("extracts Id from valid user response", function()
      ...
      local user_id, err = DebugUtils.parse_user_data(json)
      eq("005000000000001", user_id)
      eq(err, nil)
    end)

    it("returns error for malformed JSON", function()
      local id, err = DebugUtils.parse_user_data("not json")
      eq(id, nil)
      expect.no_equality(err, nil)
    end)
  end)
