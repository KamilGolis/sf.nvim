...
describe("org-connect", function()
  ...
  describe("OrgUtils.process_org_list", function()
    ...
    it("parses valid org list JSON", function()
      ...
      eq(ok, true)
      expect.no_equality(orgs, nil)
      eq(3, #orgs)
      eq("prod", orgs[1].text)
      eq("admin@mycompany.com", orgs[1].description)
      eq("https://na1.salesforce.com", orgs[1].details)
      eq(orgs[1].org_data.isDefault, true)
    end)
  ...
  describe("OrgUtils.create_org_selection_picker", function()
    ...
    it("creates picker with org items", function()
      local orgs = {
        {
          text = "dev",
          description = "dev@test.com",
          details = "https://test.sf.com",
          org_data = { username = "dev@test.com" },
        },
      }
      local cb = function() end
      OrgUtils.create_org_selection_picker(orgs, cb)
      eq(mock_snacks.assert_called(), true)
  ...
  describe("OrgUtils.set_target_org", function()
    ...
    it("spawns config set command and invokes callback on success", function()
      local called = false
      OrgUtils.set_target_org({ username = "test-user@example.com", alias = "dev" }, nil, function(success, msg)
        called = true
        eq(success, true)
      ...
      eq(mock_vim.assert_system_called(), true)
      eq(called, true)
      local calls = mock_vim.get_system_calls()
      local args = calls[1].args
      expect.no_equality(args, nil)
    end)
  end)
    ...
    before_each(function()
      ...
      Helper.mock_vim.setup_fn_mocks({
        ...
        nvim_buf_get_name = function()
          return tmpdir .. "/classes/FakeClass.cls"
        end,
      })
    end)
  ...
  describe("Connect:select_default_org", function()
    ...
    it("fetches org list and creates picker", function()
      ...
      local mock_vim = require("tests.helpers.mock_vim")
      Helper.wait_for(function()
        return #mock_vim.get_system_calls() > 0
      end)
      eq(#mock_vim.get_system_calls() > 0, true)
    end)
  end)
end)
