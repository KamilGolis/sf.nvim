describe("metadata-deploy", function()
  ...
  after_each(function()
    if _G._sf_deploy_tmpdir then vim.fn.delete(_G._sf_deploy_tmpdir, "rf"); _G._sf_deploy_tmpdir = nil end
    Helper.teardown()
  end)
  ...
  describe("DeployUtils.create_deployment_context", function()
    ...
    it("creates context for current file deploy", function()
      local ctx = DeployUtils.create_deployment_context("current_file", "/p/FakeClass.cls", nil, { api_version = "65.0" })
    ...
    it("creates context for multi-file deploy", function()
      local ctx = DeployUtils.create_deployment_context("changed_files", nil, { "a.cls", "b.cls" }, { api_version = "65.0" })
  ...
  describe("DeployUtils.detect_source_conflict", function()
    ...
    it("detects SourceConflictError", function()
      local is_conflict, msg = DeployUtils.detect_source_conflict({
        name = "SourceConflictError", message = "Conflict on FakeClass",
      })
    ...
    it("returns false for non-conflict error", function()
      local is_conflict, msg = DeployUtils.detect_source_conflict({
        name = "GenericError", message = "Something went wrong",
      })
  ...
  describe("DeployUtils.extract_component_failures", function()
    ...
    it("extracts failures from deploy result", function()
      local deploy_result = {
        result = { details = { componentFailures = {
          { fullName = "FakeClass", fileName = "classes/FakeClass.cls", problemType = "Error", lineNumber = 42, columnNumber = 10, componentType = "ApexClass" },
        }, }, files = {}, },
      }
    ...
    it("handles failures without line/column", function()
      local deploy_result = {
        result = { details = { componentFailures = {
          { fullName = "FakeClass", fileName = "classes/FakeClass.cls", problemType = "Error", componentType = "ApexClass" },
        }, }, files = {}, },
      }
      local results = DeployUtils.extract_component_failures(deploy_result)
      assert.is_not_nil(results["FakeClass"])
