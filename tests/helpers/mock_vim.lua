...
local function make_mock_system_job(...)
  ...
  if type(call_args[1]) == "table" and type(call_args[2]) == "table" then
    ...
    cmd_args = {}
    for i = 2, #call_args[1] do
      table.insert(cmd_args, call_args[1][i])
    end
    opts = call_args[2] or {}
  ...
  function job_handle:result()
    return self._stdout
  end
  function job_handle:stderr_result()
    return self._stderr
  end
  function job_handle:start() end
  function job_handle:shutdown() end
  function job_handle:wait()
  ...
  schedule(function()
    if opts.on_exit then
      opts.on_exit(job_handle, exit_code)
    end
  end)
  schedule(function()
    if opts.on_success and exit_code == 0 then
      opts.on_success(job_handle, exit_code)
    end
  end)
  schedule(function()
    if opts.on_error and exit_code ~= 0 then
      opts.on_error(job_handle, exit_code)
    end
  end)
...
function M.setup_fn_mocks(overrides)
  if not _orig.vim_fn_exepath then
    _orig.vim_fn_exepath = vim.fn.exepath
  end
  if not _orig.vim_fn_fnamemodify then
    _orig.vim_fn_fnamemodify = vim.fn.fnamemodify
  end
  if not _orig.vim_fn_filereadable then
    _orig.vim_fn_filereadable = vim.fn.filereadable
  end
  if not _orig.vim_fn_getcwd then
    _orig.vim_fn_getcwd = vim.fn.getcwd
  end
  if not _orig.vim_fn_sign_place then
    _orig.vim_fn_sign_place = vim.fn.sign_place
  end
  if not _orig.vim_fn_sign_unplace then
    _orig.vim_fn_sign_unplace = vim.fn.sign_unplace
  end
  if not _orig.vim_fn_sign_define then
    _orig.vim_fn_sign_define = vim.fn.sign_define
  end
  if not _orig.vim_fn_readfile then
    _orig.vim_fn_readfile = vim.fn.readfile
  end
  if not _orig.vim_fn_mkdir then
    _orig.vim_fn_mkdir = vim.fn.mkdir
  end
  if not _orig.vim_fn_delete then
    _orig.vim_fn_delete = vim.fn.delete
  end
  if not _orig.vim_fn_localtime then
    _orig.vim_fn_localtime = vim.fn.localtime
  end
  ...
  if overrides.fnamemodify ~= nil then
    vim.fn.fnamemodify = function(path, modifier)
      ...
      return type(overrides.fnamemodify) == "function" and overrides.fnamemodify(path, modifier)
        or overrides.fnamemodify
    end
  end
  ...
  if overrides.nvim_buf_get_name ~= nil then
    ...
    vim.api.nvim_buf_get_name = function(bufnr)
      ...
      return type(overrides.nvim_buf_get_name) == "function" and overrides.nvim_buf_get_name(bufnr)
        or overrides.nvim_buf_get_name
    end
  end
...
function M.restore()
  if _orig.vim_system then
    vim.system = _orig.vim_system
  end
  if _orig.vim_notify then
    vim.notify = _orig.vim_notify
  end
  if _orig.vim_fn_exepath then
    vim.fn.exepath = _orig.vim_fn_exepath
  end
  if _orig.vim_fn_fnamemodify then
    vim.fn.fnamemodify = _orig.vim_fn_fnamemodify
  end
  if _orig.vim_fn_filereadable then
    vim.fn.filereadable = _orig.vim_fn_filereadable
  end
  if _orig.vim_fn_getcwd then
    vim.fn.getcwd = _orig.vim_fn_getcwd
  end
  if _orig.vim_fn_sign_place then
    vim.fn.sign_place = _orig.vim_fn_sign_place
  end
  if _orig.vim_fn_sign_unplace then
    vim.fn.sign_unplace = _orig.vim_fn_sign_unplace
  end
  if _orig.vim_fn_sign_define then
    vim.fn.sign_define = _orig.vim_fn_sign_define
  end
  if _orig.vim_fn_readfile then
    vim.fn.readfile = _orig.vim_fn_readfile
  end
  if _orig.vim_fn_mkdir then
    vim.fn.mkdir = _orig.vim_fn_mkdir
  end
  if _orig.vim_fn_delete then
    vim.fn.delete = _orig.vim_fn_delete
  end
  if _orig.vim_fn_localtime then
    vim.fn.localtime = _orig.vim_fn_localtime
  end
...
function M.get_system_calls()
  return M.calls.system
end
